var App={
    init(){
        ...
        <div class="nav-blur pb-safe h-[68px] flex items-center justify-around fixed bottom-0 w-full z-40 px-2 ">
            ...
            <button onclick="App.switch('profile')" id="nav-profile" class="nav-item group relative flex-1 h-full flex flex-col items-center justify-center cursor-pointer select-none touch-manipulation active:scale-95 transition-all">
                <div class="nav-icon-wrapper w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300">
                    <i data-lucide="user" class="w-5 h-5"></i>
                </div>
                <span class="nav-label text-[10px] font-bold transition-all duration-300">Profil</span>
            </button>
        </div>`;

        gid('view-profile').innerHTML=`
        <div class="pt-12 px-4 pb-6 text-center">
            <div id="profile-account-section" class="max-w-sm mx-auto mb-6 text-left"></div>

            <div class="glass rounded-2xl p-5 max-w-sm mx-auto space-y-3 text-left mb-6">
                <h3 class="text-white font-bold text-sm uppercase tracking-wider mb-2">Tentang Aplikasi</h3>
                <div class="flex justify-between"><span class="text-white/70 text-sm">Nama</span><span class="text-white font-medium text-sm">Beatsify</span></div>
        ...
        if(typeof Auth!=='undefined' && Auth.renderProfile) Auth.renderProfile();
    ...
    autoPlayTrack(videoId){
        fetch(API.search+'?query=https://youtube.com/watch?v='+videoId).then(function(r){return r.json();}).then(function(d){
            var title='Lagu',artist='Beatsify',cover=toHDCover('', videoId),artistId='';
            ...
            S.ct={id:videoId,videoId:videoId,title:'Lagu',artist:'Beatsify',cover:toHDCover('', videoId),artistId:'',ytUrl:'https://youtube.com/watch?v='+videoId};
            S.ps='direct';S.pl=[S.ct];S.pi=0;UU();MP.show();resetLyricsUI(videoId);
            setTimeout(function(){FullPlayer.open();loadTrack(S.ct);},400);
        });
    },
    showSharePopup(videoId){
        fetch(API.search+'?query=https://youtube.com/watch?v='+videoId).then(function(r){return r.json();}).then(function(d){
            var title='Lagu',artist='Beatsify',cover=toHDCover('', videoId);
        ...
        }).catch(function(){App.renderPopup(videoId,'Lagu','Beatsify',toHDCover('', videoId));});
    },
    ...
    switch(t){
        ...
        document.querySelectorAll('.fixed.z-\\[300\\], .fixed.z-\\[400\\]').forEach(function(el){
            if(el.id !== 'mini-player') el.remove();
        });
        ...
        if(t==='profile'){if(typeof Auth!=='undefined' && Auth.renderProfile) Auth.renderProfile();}
        ...
        ['home','search','library','profile'].forEach(function(n){
            var b=gid('nav-'+n);
            if(!b)return;
...
var Library={
    ...
    open(id){
        ...
        if(!modal) {
            ...
            modal.className = 'fixed inset-0 bg-[#000000] flex flex-col z-[100]';
